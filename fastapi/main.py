...
async def authentication_exception_handler(request, exc):
    return JSONResponse(
        status_code=401,
        content=jsonable_encoder({"detail": "Authentication failed. Please check your API key."})
    )
...
async def process_filings(
    ...
    if not is_api_key_valid(openai_key):
        return JSONResponse(content={"message": "ERROR: Cannot Verify Your OpenAI API key. Please check the network or your OPENAI key"})
